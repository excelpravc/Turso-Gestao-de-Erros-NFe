...
//
//  Como funciona AGORA (pós-migração pra Turso):
//  - Não existe mais window.dbCentral como conexão de banco — o
//    diretório de clientes mora no banco Turso "admin", acessado só
//    pelo SERVIDOR (api/admin/usuarios.js, api/auth/login.js). O
//    browser nunca vê a URL/token de banco de ninguém.
//  - Ao logar como cliente normal, o servidor confere a senha e
//    devolve o `id` do tenant. Chamamos _initTenantFirebase(id)
//    (nome mantido por compatibilidade — está em turso-init.js) só
//    pra guardar esse id em window.dbTenant/window.CURRENT_TENANT_ID.
//    Daí em diante todo o sistema (histórico, cadastros etc.) lê e
//    grava via polyfill.js, que já manda o tenantId em toda chamada.
//  - Ao logar como usuário com isAdmin:true, abre o Painel Admin em
//    vez do sistema normal — CRUD via /api/usuarios.
//
//  IMPORTANTE — bootstrap do primeiro usuário ADM:
//  Antes isso era feito direto no Console do Firebase. Agora:
//    turso db shell errosnfe-admin
//    INSERT INTO usuarios (id, usuario, senha, isAdmin, ativo)
//    VALUES ('admin1', 'admin', 'escolha-uma-senha-forte', 1, 1);
//  Depois disso, logins seguintes (novos clientes) podem ser
//  cadastrados direto pelo Painel Admin, sem precisar mexer no
//  banco de novo.
//
//  Precisa carregar DEPOIS de: turso-init.js, polyfill.js, scripts.js
// ════════════════════════════════════════════════════════════════
...
async function fazerLogin() {
  ...
  try {
    ...
    window.CURRENT_USUARIO_ID = r.usuario.id;
    await _initTenantFirebase(r.usuario.id); // nome mantido por compatibilidade — está em turso-init.js

    if (modo === 'edit') {
      window._MODO_VISUALIZACAO = false;
      SENHA_EDICAO = r.senhaEdicaoAtual;
    } else {
...
async function _renderListaUsuarios() {
  ...
  try {
    ...
    if (ativosEl) ativosEl.textContent = clientes.filter(u => u.ativo).length;
    if (inativosEl) inativosEl.textContent = clientes.filter(u => !u.ativo).length;
    ...
    box.innerHTML = usuarios.map(u => {
      ...
      const corAvatar = u.isAdmin ? 'var(--accent)' : (!u.ativo ? 'var(--muted)' : '#4d9fff');
      const detalhes = u.isAdmin
        ...
        : `<span style="color:var(--muted)">usuário:</span> ${esc(u.usuario)}` +
          ...
          (u.tursoUrl ? `<br><span style="color:var(--muted)">banco:</span> ${esc(u.tursoUrl)}` : '');
      return `
        ...
        <div style="display:flex;gap:14px;align-items:center;min-width:0">
          ...
          <div style="min-width:0">
            <div style="font-weight:700;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
              ...
              ${!u.ativo ? '<span style="background:var(--danger);color:#fff;font-size:.6rem;font-weight:800;letter-spacing:.5px;padding:2px 8px;border-radius:20px">INATIVO</span>' : ''}
            </div>
...
async function abrirFormUsuario(id) {
  ...
  ['usu-usuario', 'usu-senha', 'usu-empresa', 'usu-tursoUrl', 'usu-tursoToken', 'usu-senha-sistema']
  ...
  if (id) {
    try {
      const resp = await fetch('/api/usuarios?id=' + encodeURIComponent(id));
      const r = await resp.json();
      if (r.ok && r.usuario) {
        const d = r.usuario;
        ...
        document.getElementById('usu-ativo').checked = !!d.ativo;
        document.getElementById('usu-tursoUrl').value = d.tursoUrl || '';
        document.getElementById('usu-tursoToken').value = d.tursoToken || '';
        ...
        if (d.isAdmin) {
          ...
          if (d.tursoUrl) {
            try {
              const senhaAtualReal = await _lerSenhaSistemaDoTenant(id);
              ...
              senhaInp.placeholder = 'Não foi possível ler (verifique se o banco Turso desse cliente está ativo)';
            }
          } else {
            senhaInp.placeholder = 'Este cliente ainda não tem banco Turso configurado';
          }
        }
      }
...
// Lê a senha do sistema (edição) direto do banco do próprio cliente, via API.
// Antes abria uma conexão Firebase temporária no browser; agora é o
// servidor quem resolve tursoUrl/tursoToken e consulta — o browser nunca
// vê o token do banco do cliente.
async function _lerSenhaSistemaDoTenant(tenantId) {
  const resp = await fetch('/api/config?tenantId=' + encodeURIComponent(tenantId) + '&chave=sistema');
  const r = await resp.json();
  if (!r.ok) throw new Error(r.error || 'falha ao ler config do tenant');
  return (r.valor && r.valor.senha) || '@mudar';
}
...
async function salvarUsuario() {
  ...
  const tursoUrl = document.getElementById('usu-tursoUrl').value.trim();
  const tursoToken = document.getElementById('usu-tursoToken').value.trim();

  if (!usuario || !senha) { toast('Preencha usuário e senha!', true); return; }

  const dados = { usuario, senha, empresa, ativo, tursoUrl, tursoToken };

  try {
    // Se o ADM preencheu uma nova senha do sistema (edição), grava ela
    // DIRETO no banco Turso do próprio cliente — não só no espelho central.
    if (id && novaSenhaSistema && tursoUrl) {
      await _escreverSenhaSistemaNoTenant(id, novaSenhaSistema);
...
// Grava a nova senha do sistema direto no banco Turso do cliente, via API
// (equivalente a abrir uma conexão Firebase temporária e fechar em seguida).
async function _escreverSenhaSistemaNoTenant(tenantId, novaSenha) {
  const resp = await fetch('/api/config?tenantId=' + encodeURIComponent(tenantId), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chave: 'sistema', valor: { senha: novaSenha }, merge: true })
  });
  const r = await resp.json();
  if (!r.ok) throw new Error(r.error || 'falha ao gravar senha no tenant');
}
...
async function excluirUsuario(id, nomeExibicao) {
  const ok = await confirmModal({
    ...
    desc: `Deseja realmente excluir <strong style="color:var(--text)">${esc(nomeExibicao || '')}</strong>?<br><br>Isso remove só o ACESSO dele ao login. NÃO apaga o banco Turso/dados dele — se quiser apagar os dados de verdade, isso precisa ser feito manualmente no dashboard do Turso.`,
    confirmLabel: '🗑 Excluir'
  });
  if (!ok) return;
  try {
    const resp = await fetch('/api/usuarios?id=' + encodeURIComponent(id), { method: 'DELETE' });
    const r = await resp.json();
...
async function alterarSenhaVisualizacao() {
  ...
  try {
    const resp = await fetch('/api/trocar-senha', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: window.CURRENT_USUARIO_ID, atual, nova })
    });
    const r = await resp.json();
    if (!r.ok) {
      toast(r.msg || 'Senha atual incorreta!', true);
      return;
    }
