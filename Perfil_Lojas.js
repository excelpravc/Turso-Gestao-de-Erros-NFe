...
// ════════════════════════════════════════════════════════════════
//  PERFIL_LOJAS — Configurações e overrides do perfil Lojas
//  - Histórico: Historico_Lojas
//  - Campo Loja: dropdown normal com lista da planilha
//  - Menu lateral: mostra aba "Lojas"
//  - Dashboard NFS: habilitado
...
function _aplicarPerfilLojas() {
// 1. Mostra aba "Lojas" no menu lateral (visível só para este perfil)
const tabLoja = document.querySelector('.sb .tab[onclick*="p-loja"]');
if (tabLoja) tabLoja.style.display = '';

// 2. Loja no formulário: comportamento normal (dropdown habilitado)
//    Não faz nada — o padrão do sistema já é o dropdown normal
//    A função applyLojaFixa() do Scripts.html lida com loja fixa se configurada

// 3. Campo loja no dashboard lateral: label padrão
const lblKpiLoja = document.getElementById('kpi-loja');
if (lblKpiLoja) {
const card = lblKpiLoja.closest('.kpi-card');
if (card) {
const lbl = card.querySelector('.kpi-lbl');
if (lbl) lbl.textContent = 'Lojas Impactadas';
}
}

// 4. Títulos dos gráficos corretos para o perfil Lojas
const ctErrosPie = document.getElementById('ct-erros-pie');
if (ctErrosPie) ctErrosPie.textContent = '🏪 Ranking Lojas';
const ctLojas = document.getElementById('ct-lojas');
if (ctLojas) ctLojas.textContent = '⚠️ Ranking Erros';
}

// ── Override: buildSelects — popula sel_loja normalmente ──
// (o Scripts.html já faz isso por padrão, nenhum override necessário)

// ── Override: applyLojaFixa — comportamento normal ──
// (Scripts.html já implementa; aqui garantimos que NÃO está bloqueado)
function _perfilOverrideLojaFixa() {
const selLoja = document.getElementById('sel_loja');
if (!selLoja) return;
// Garante que o select NÃO está desabilitado pelo perfil Matriz
selLoja.disabled = false;
selLoja.style.opacity = '';
selLoja.style.cursor = '';
}

// Chama o override depois que o sistema carregar
document.addEventListener('_sistemaCarregado', _perfilOverrideLojaFixa);

// ════════════════════════════════════════════════════════════════
//  GRÁFICOS INDEPENDENTES — PERFIL LOJAS
...
// ── Rankings ──
window._renderRankings_Lojas = function(r) {
renderRank('rl-forn',      countBy(r, 'fornecedor'), '#4d9fff');
renderRank('rl-erros',     countBy(r, 'erroDesc'),   '#f5a623');

// Ao lado da rosca: Ranking LOJAS (todas, sem limite)
const ctErrosPie = document.getElementById('ct-erros-pie');
if(ctErrosPie) ctErrosPie.textContent = '🏪 Ranking Lojas';
const lojasData = countBy(r,'loja');
const rlErrosPie = document.getElementById('rl-erros-pie');
if(rlErrosPie){
if(!lojasData.length){
rlErrosPie.innerHTML='<div class="nd">Sem dados no período</div>';
} else {
const max = lojasData[0][1];
rlErrosPie.innerHTML = lojasData.map((d,i)=>{
const pct = Math.max(15, Math.round(d[1]/max*100));
return `<div class="ri">
<span class="ri-pos">${i+1}</span>
<div class="ri-bw">
<div class="ri-b" style="width:${pct}%;background:#a78bfa;color:#fff">
<span class="ri-b-label">${esc(d[0])}</span>
</div>
</div>
<span class="ri-n" style="color:#a78bfa">${d[1]}</span>
</div>`;
}).join('');
}
}

// Seção inferior: Ranking ERROS
const ctLojas = document.getElementById('ct-lojas');
if(ctLojas) ctLojas.textContent = '⚠️ Ranking Erros';
renderRank('rl-lojas', countBy(r,'erroDesc'), '#f5a623');
